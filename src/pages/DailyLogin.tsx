    ...
    <div className="flex flex-col px-5 md:px-8 lg:px-10 pt-7 md:pt-10 pb-2 gap-5 md:gap-7">
      ...
      <div className="glass-card rounded-3xl p-6 md:p-8 flex flex-col items-center gap-4 md:gap-5 shadow-[var(--shadow-elevated)]">
        ...
        <div className="text-center">
          ...
          >
            <AnimatedNumber value={streak} className="text-[32px] md:text-[42px] font-extrabold text-gradient-coral leading-none inline-block" />
          </motion.div>
          <p className="text-[14px] md:text-[16px] font-semibold text-ink-secondary mt-1">Day Streak</p>
          <p className="text-[11px] md:text-[13px] text-ink-muted flex items-center justify-center gap-1 mt-2">
            <Target size={12} className="text-coral" /> Next milestone: 7-day streak
          </p>
        </div>
      ...
      >
        ...
        <div className="flex-1">
          <p className="text-[15px] md:text-[17px] font-bold text-ink">Jackpot — {day7.label}</p>
          <p className="text-[11px] md:text-[13px] text-ink-muted mt-0.5">Complete a full week</p>
        </div>
      ...
      <div className="bg-violet-light rounded-2xl p-4 md:p-5 flex items-center gap-3.5">
        ...
        <div>
          <p className="text-[13px] md:text-[15px] font-bold text-ink">7-day streak = 2x rewards</p>
          <p className="text-[11px] md:text-[13px] text-ink-muted mt-0.5">Keep logging in daily</p>
        </div>
      </div>
      ...
      <div className="glass-card rounded-2xl p-5 md:p-6 shadow-[var(--shadow-soft)]">
        ...
        <div className="flex justify-around">
          <Stat icon={<Coins size={20} className="text-gold" />} val={60} label="Earned" />
          <div className="w-px bg-muted" />
          <Stat icon={<Flame size={20} className="text-coral" />} val={streak} label="Streak" />
          <div className="w-px bg-muted" />
          <Stat icon={<Rocket size={20} className="text-violet" />} val="1x" label="Multi" />
        </div>
      </div>
    </div>
  )
...
function Stat({ icon, val, label }: { icon: React.ReactNode; val: number | string; label: string }) {
  return (
    <div className="flex flex-col items-center gap-1.5">
      {icon}
      {typeof val === 'number'
        ? <AnimatedNumber value={val} className="text-[18px] md:text-[22px] font-extrabold text-ink" />
        : <span className="text-[18px] md:text-[22px] font-extrabold text-ink">{val}</span>
      }
      <span className="text-[9px] md:text-[11px] font-semibold text-ink-muted uppercase tracking-[0.08em]">{label}</span>
    </div>
  )
}
