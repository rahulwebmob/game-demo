...
function genSequence(level: number): { seq: number[]; answer: number; missingIdx: number } {
    ...
    return (
      ...
      >
        ...
        <motion.button
          ...
          style={{ boxShadow: '0 4px 16px rgba(77,163,232,0.3)' }}
        >
          ...
          >
            {i === problem.missingIdx ? (
              <div className="w-12 h-12 md:w-14 md:h-14 rounded-xl border-2 border-dashed border-coral flex items-center justify-center">
                {selected !== null ? (
                  <span className={`text-[16px] md:text-[18px] font-bold ${selected === problem.answer ? 'text-green' : 'text-rose'}`}>
                    {problem.answer}
                  </span>
                ) : (
              ...
              <div className="w-12 h-12 md:w-14 md:h-14 rounded-xl bg-muted flex items-center justify-center">
                <span className="text-[16px] md:text-[18px] font-bold text-ink tabular-nums">{num}</span>
              </div>
            )}
          </motion.div>
        ))}
          ...
          return (
            <motion.button
              ...
              className={`py-4 rounded-2xl border-2 font-bold text-[18px] cursor-pointer transition-colors ${
                isCorrect ? 'bg-green-light border-green text-green' :
                isWrong ? 'bg-rose-light border-rose text-rose' :
                selected !== null ? 'bg-muted border-transparent text-ink-muted' :
