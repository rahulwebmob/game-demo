...
function ThemeCircle({ id, size = 24, ring }: { id: ThemeId; size?: number; ring?: string }) {
    ...
    <div className="relative" ref={ref}>
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11px] md:text-[12px] text-ink-muted">{themeMeta[themeId].label}</p>
        </div>
        <div className="flex items-center gap-2">
          <ThemeCircle id={themeId} />
          <span className="text-[12px] md:text-[13px] font-semibold text-ink-secondary">{themeMeta[themeId].label}</span>
            ...
            {(Object.keys(themeMeta) as ThemeId[]).map(id => {
              ...
              return (
                <motion.button
                  key={id}
                  whileTap={{ scale: 0.97 }}
                  onClick={() => { sfx('toggle', true); onThemeChange(id); setOpen(false) }}
                  className={`flex items-center gap-3 px-3 py-2 rounded-xl border-none cursor-pointer transition-colors w-full text-left ${
                    active ? 'bg-coral-light' : 'bg-transparent hover:bg-muted/50'
                  }`}
                >
                  <ThemeCircle id={id} ring={active ? '0 0 0 2px var(--color-coral)' : undefined} />
                  <span className={`text-[12px] md:text-[13px] font-semibold flex-1 ${active ? 'text-coral' : 'text-ink'}`}>
                    {themeMeta[id].label}
                  </span>
                  {active && <Check size={14} className="text-coral flex-shrink-0" />}
                </motion.button>
              )
            })}
          </motion.div>
